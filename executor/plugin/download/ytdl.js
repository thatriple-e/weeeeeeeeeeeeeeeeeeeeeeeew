...
module.exports = {
    ...
    operate: async (context) => {
        const { tdx, m, q, prefix, command, reaction, xreply, getRandom, Format } = context;

        if (!q) {
            await xreply(`Add input, Example: *${prefix + command} https://youtu.be/7U20i3bMX10?si=5MwAl2F_mB98SXfZ*`);
        ...
        async function ytmp3(ptz, m, url) {
            try {
                ...
                let a = path.resolve(__dirname, "../../../trashbase/src/" + randomBytes(4).toString('hex') + ".mp3");
                ...
                await tdx.sendMessage(m.chat, { audio: fs.readFileSync(a), mimetype: 'audio/mpeg', ptt: false }, { quoted: m });
            } catch (e) {
                console.error(e);
                return {
                    status: 'error',
...
\`\`\``
            ...
            await xreply('Failed to download the YouTube video. Please try again later.');
            await reaction(m.chat, "❌");
        }
    }
};